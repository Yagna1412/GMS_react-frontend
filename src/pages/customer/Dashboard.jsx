...
// Optional interceptor (same as your file)
  ...
  useEffect(() => {
    ...
    const fetchData = async () => {
      try {
        ...
        const history = await api.get("/dashboard/spending-history");
        const breakdown = await api.get("/dashboard/service-breakdown");
        const recent = await api.get("/services/recent");
        const vehicle = await api.get("/user/vehicle");
        const profile = await api.get("/user/profile");
        const notif = await api.get("/notifications").catch(() => ({ data: [] }));


        setUser({
          name: profile.data?.name || "User",
          vehicle: {
            model: vehicle.data?.model || "No Vehicle",
            plate: vehicle.data?.number || vehicle.data?.plate || "N/A"
          },
          loyaltyPoints: summary.data?.loyaltyPoints || 0,
          totalSpent: summary.data?.totalSpent || 0,
          upcomingAppointments: summary.data?.upcoming || 0,
          activeJobs: summary.data?.activeJobs || 0
        });
        ...
        setServiceBreakdown(
          Array.isArray(breakdown.data)
            ? breakdown.data.map(item => ({
              name: item.type,
              value: item.count,
              color: SERVICE_COLORS[item.type] || "#3B82F6" // fallback
            }))
            : []
        );
        setSpendingHistory(
    ...
    <div style={{ padding: '24px', display: 'flex', flexDirection: 'column', gap: 24, maxWidth: '1400px', margin: '0 auto' }}>
      ...
      <div style={{ display: 'grid', gridTemplateColumns: 'repeat(4,1fr)', gap: 16 }}>
        {[
          ...
          { label: 'Total Spent', value: `$${user.totalSpent || 0}`, icon: CreditCard, color: '#8B5CF6', bg: '#F5F3FF' },
      ...
      <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr 1fr', gap: 20 }}>
        ...
        <Card style={{ padding: 24 }}>
          ...
          <ResponsiveContainer width="100%" height={200}>
            <LineChart data={Array.isArray(spendingHistory) ? spendingHistory : []} margin={{ top: 4, right: 8, left: -20, bottom: 0 }}>
        ...
        <Card style={{ padding: 24 }}>
          ...
          {Array.isArray(serviceBreakdown) && serviceBreakdown.length > 0 ? (
            <ResponsiveContainer width="100%" height={200}>
              <PieChart>
                <Pie
      ...
      <div style={{ display: 'grid', gridTemplateColumns: '2fr 1fr', gap: 20 }}>
        ...
        <Card style={{ padding: 24 }}>
          ...
          <table style={{ width: '100%', borderCollapse: 'collapse', fontSize: 13.5 }}>
            ...
            <tbody>
              {Array.isArray(serviceHistory) && serviceHistory.map((row, i) => (
                <tr key={i} style={{ borderBottom: '1px solid #F8FAFF' }}>
                  <td style={{ padding: '13px 10px', color: '#1E3A8A', fontWeight: 600 }}>{row.date}</td>
                  <td style={{ padding: '13px 10px', color: '#334155' }}>{row.serviceType}</td>
